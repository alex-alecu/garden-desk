...
- A top-right **Technical details** control that opens a wider right-side drawer. The drawer uses the application surface and reduces the conversation workspace instead of covering it. Its **Overview** tab contains separate live RAM and VRAM or unified-memory allocations, their total, the runtime memory budget, allocated context, hardware cap and rationale, limits, and guest capabilities. Its separate **Steps** tab contains the ordered step list, generated scripts or commands, exact committed bytes for a path-only Python or Node step, and complete execution evidence. Generated-file metadata does not appear in Technical details.
