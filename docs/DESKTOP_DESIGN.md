...
- While a step is generating, its current typed thought segment appears in a live box after the last streamed text row. When that segment ends, it stays in session-scoped desktop memory and collapses with the intermediate step. The user can open it again until Garden Desk closes. A restored application never shows it.
...
When the approved model and runtime expose a typed thought segment, the current segment may stream into a live box after the last conversation row while generation is active. The segment stays only in desktop memory and collapses with the matching intermediate activity step when generation ends. It is never restored after Garden Desk closes. Hidden or unsegmented internal reasoning is never inferred, exposed, or persisted. Activity describes observable actions and results only.
...
A build with one runnable generation model shows its human-readable name and current state in the conversation header. The model loads on first use, remains resident and ready between turns, and can be unloaded manually only while idle. Stop discards the partial response, returns the model to `ready`, and lets the next message continue the same session or start work in a different session without a model reload. A generation timeout also keeps the model resident. If the worker does not return the cancellation result within one second, Core releases the caller and ignores the late result without unloading the model. After a manual unload, the next message loads the same approved model again. Hardware detection chooses the model-plus-context budget without a settings surface. On an 8 GB Mac the header and alert explain that local inference is unsupported, and the composer cannot submit agent work. There is no picker or arbitrary configuration affordance. A future multi-model build may show only installed, signed, hardware-compatible choices.
