...
- License and redistribution: `llama.cpp` is MIT licensed, and the package includes its license text. The Windows archive also supplies the Microsoft OpenMP runtime `libomp140.x86_64.dll`. Vault Desk adds the imported `msvcp140.dll`, `vcruntime140.dll`, and `vcruntime140_1.dll` as unmodified files from the hash-pinned official Microsoft desktop runtime package and records both Microsoft runtime components under the Microsoft Software License Terms.
...
- Adapter fit: one thin TypeScript adapter owns bounded prompt scratch, process output, timeout, cancellation, final-channel parsing, and cleanup. Vault Core retains image path authority, model selection, scheduling, audit, and policy.
