...
7. Returns and durably records each tool call, path, resolved source or command, stdout, stderr, result, summary, limits, and termination reason. Text without tool calls enters final validation. Core does not parse task text or reserve a completion turn.
