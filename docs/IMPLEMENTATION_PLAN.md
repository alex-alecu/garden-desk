...
- **Tools**: `bash`, `python`, `node`, `read`, `glob`, `grep`, `list`, `write`, `edit`, `image`, `skill`, `task`, and `question`. `read`, `glob`, `grep`, and `list` inspect the guest; `write` and `edit` change files only under `/workspace`; `bash`, `python`, and `node` run inside the guest. `task` selects a packaged child agent by its name and description. The main agent can select a specialist for a separate body of work. Children run one at a time in the same session and VM. Specialized instructions load on demand through `skill`.
...
Fixed specialists use packaged agent Markdown and existing format and subject skills. Folder intake identifies current file groups and structures before batch work. Matter chronology builds a cited timeline across legal records. Contract obligations extracts duties and terms from agreements and amendments; `/obligations` selects that same workflow. Document comparison reports changes across supplied versions with references to each source. Delegated specialists return cited findings and working evidence under `/workspace/.garden-desk-tools/<child-run-id>/`; the main agent owns final files. Each child receives the assigned request and attachment metadata, without the full conversation. Its saved identity, assignment, and parent tool-call ID link its activity to the parent. Parent snapshots include child summaries; completed child findings remain available after restart. Partial response text stays in memory.
...
This explicit workflow is the exception to the generic format-routing rule. It adds no automatic task classification or host document processing.
