...
7. Returns and durably records each tool call, path, source or command, stdout, stderr, result, summary, workspace changes, limits, and termination reason. Text without tool calls enters final validation. A task-named output must be a safe current-run artifact. Core gives one generic recovery when it is absent, then returns a stable failure.
...
The guest has no package manager authority, credentials, user home, host shell, general Vault Core endpoint, generic model server, approval authority, export authority, or network broker. Its `/bin/sh` is inside the no-network guest and has no host authority. Host source folders are never writable. At successful finalization, Core hashes and persists verified regular files created or changed by successful executions, excluding internal tool state and intermediate code. Opening uses a verified owner-only temporary copy; Save As uses a native dialog and an atomic Core write without returning the selected destination path to the webview.
...
- If a turn reaches its output limit without a tool call, Core discards it and requests one immediate typed tool call or concise answer. It keeps tools available and compacts only when measured prompt use is also at least 80 percent of the context allocation. A second output-limit turn fails with `agent_generation_limit`. Successful runs independently expose the latest safe non-internal files observed in workspace deltas. A task-named output must be a safe current-run artifact; Core gives one generic recovery, then returns a stable failure. No model-authored declaration, format-specific completion marker, suffix allowlist, or format allowlist is required.
...
Stage state: earlier macOS and Windows physical evidence is historical. The current reliability change has focused coverage and a current Stage 2 context-session stress result in [STRESS_TEST.md](../STRESS_TEST.md). Current physical Windows Stage 3 and Stage 4 evidence is pending. It must not be inferred from earlier gates, focused coverage, or Stage 2. Generated-file Open and Save As observations on both packaged platforms, dedicated-standard-user setup, macOS lower-tier context, and release-signing observations also remain open.
...
- Keep the generic `read` tool as strict streamed UTF-8 plain-text inspection. Invalid UTF-8, NUL bytes, and invalid numeric arguments fail. Limit every model-facing tool-result preview to 50 KiB after JSON encoding and retain complete oversized output in the internal spill path.
- Require a safe current-run artifact for every task-named output. Permit one generic recovery, then return a stable failure. Exclude internal paths and do not add a suffix or format allowlist.
