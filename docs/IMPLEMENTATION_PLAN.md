...
- The Core-owned loop sends persistent conversation history and generic tool definitions to the worker for each native chat turn. A cold run requests automatic context and keeps that request shape stable while budgeting from the worker's measured allocation. It permits 40 model turns and 24 started guest executions. Core validates a tool call before it can use this budget. Unsupported model input returns a failed tool result to the next turn and does not start a guest process. Safe optional integers clamp to their configured bounds; wrong types, non-finite numbers, and unsafe integers fail. The generic `read` tool streams strict UTF-8 plain text and fails for invalid UTF-8 and NUL bytes. Tool-result previews retain at most 2,000 lines and 50 KiB after JSON encoding. Complete output spills under `/workspace/.vault-output` only when all chunks fit in the remaining guest budget; otherwise no spill process starts and the completed primary execution remains recorded. Chat output uses up to 16,384 tokens while reserving at least 4,096 tokens for the prompt, and compact summaries stay bounded at 2,048. An explicit question-tool test makes the first turn one question call without a planning turn.
- Duplicate recovery uses a stable tool signature, which is the tool name plus normalized input. Core blocks the third identical call and each later identical call without guest budget use. An already-loaded skill is the only exception: the call returns the existing zero-cost result and does not enter duplicate recovery. Later model prompts keep the second call and its complete bounded result. They omit the first, third, and later redundant pairs. Trace, audit, event, and execution records stay complete. One replaceable system instruction, which states how many identical calls were made and rejected and the last error line of the retained result, and one replaceable latest user direction guide recovery. Model turns issued while a block is active use temperature 0.3, so a greedy repeat can change; every other turn keeps the agent temperature. A different validated call must execute and return new success or failure evidence before Core clears the active block. After two later turns that contain only the blocked call, Core asks one question with the header `Repeated action`: `The same action is still repeated after automatic recovery. How should Vault Desk continue?` The options are `Inspect first (Recommended)` with `Use a different inspection action before another execution.`, and `Change method` with `Use another available tool or different input.` A dismissed question gives the direction `Inspect current state before another execution.` If the model ignores the direction for two more turns that contain only the blocked call, if questions are unavailable, or if no model turn remains, the run fails with `agent_stalled_duplicate`.
- Measured prompt use at 80 percent of the allocation triggers a no-tool anchored summary while preserving the current user request and last two assistant/tool turns verbatim. Core appends a bounded data-only record with current-run named script paths and the last failed execution termination, exit code, and error text. It does not extract task facts. Different failed calls remain in live context. Duplicate recovery gives compaction the cleaned history without its temporary system instruction. After compaction, Core removes obsolete omitted-call identifiers but keeps the active block and latest user direction. Durable conversation, execution, trace, and audit records are unchanged by compaction.
...
- A Python or Node call accepts a relative workspace path, a normalized `/workspace/...` path, or a path-only `/source/...` path. Its tool description puts these namespaces on the `path` parameter. Workspace path-only calls run the last committed bytes and record the resolved source. Direct source calls run the live mounted file and record its absolute path with null source text. Missing source files and interpreter errors return normal nonzero tool results. Unsupported namespaces and traversal return failed tool results without guest execution or task termination.
