...
5. Atomically writes complete Python source or Node `.mjs` ES-module source to a safe workspace-relative path, or runs a command shorter than 4,096 characters through `/bin/sh` from `/workspace`. The structured grammar accepts one complete command without a grammar-level string cutoff, and Core validates the total before execution so a boundary-length result is treated as potentially truncated and rejected. Core also rejects a shell proposal that embeds Python or Node source, or source containing model-protocol fragments, before it reaches the VM. A shell parse failure caused by an unterminated quoted string forces the next action into Python or Node source mode instead of asking for another shell command. Product-owned skills route declaratively from trusted task text, attachment names, declared command or source paths, and validated typed skill requests. When an active skill declares a progress contract, later turns enter the generic source-only progress workflow and require complete verified coverage independently of other formats.
...
7. Returns and durably records the proposal, path, source or command, stdout, stderr, result, summary, workspace changes, limits, and termination reason. A final `respond` action may declare a unique bounded set of observed current-run workspace paths as requested deliverables.
...
9. Keeps idle VMs in a least-recently-used pool bounded by total RAM after the inference cap and host reserve. Different conversations may run concurrently up to that capacity; their model turns queue through the one resident Gemma worker. Excess runs remain queued without starting another VM.

The guest has no package manager authority, credentials, user home, host shell, general Vault Core endpoint, generic model server, approval authority, export authority, or network broker. Its `/bin/sh` is inside the no-NIC guest and has no host authority. Host source folders are never writable. At successful finalization, Core hashes and persists only valid declared deliverables. Opening uses a verified owner-only temporary copy; Save As uses a native dialog and an atomic Core write without returning the selected destination path to the webview.
...
- Catalog schema v12 preserves the v7 normalized execution records, v8 versioned inference turns, and v9 persistent owner-defined folder ordering, adds the validated typed skill-request and unbacked-response outcomes, and stores one replaceable anchored session summary per session. New runs durably capture the exact effective prompt, canonical JSON schema, exact pre-parse structured result, worker request identifiers, limits, decision outcome, and execution sequence through immutable content hashes. Historical runs remain version 0 and explicitly report `not_recorded`; restart recovery marks unfinished turns `interrupted`.
...
- Conversation context is rebuilt from durable messages and execution events against the worker's actual reported allocation. Core requests at most 32,768 output tokens for a new executable proposal and reserves up to that amount while keeping at least 4,096 tokens for the current prompt; failed or rejected repairs plus XLSX repair and continuation turns use 8,192 tokens, and forced final responses use 4,096. The certified 8K minimum therefore reserves 4,096 output tokens. Core protects the newest failed repair source exactly, anchors up to two recent user turns, and compacts older history without deleting originals. Successful current-run source, commands, stdout, and stderr share an allocation-derived live budget; overflow produces inspectable task, evidence, artifact, and warning ledgers while exact execution bytes remain durable. Each of the six possible executions retains a bounded ranked evidence share so later turnover cannot displace an earlier explicit identifier or label. A completed step is not rerun merely because its raw context left the live prompt. A generation-limit stop gets one newly traced 8,192-token recovery request for a small complete Python or Node action that incrementally creates or patches a persistent workspace file; repeated exhaustion fails explicitly without executing partial output.
- Nonempty exit-zero output retained through the typed `resource_limit` boundary may be finalized without forcing another source execution; empty or nonzero resource failures, crashes, timeouts, and stderr still require repair. This keeps the one-million-byte output cap fail-safe without making a correctly bounded execution loop forever.
- When verbatim older conversation exceeds the allocation-derived history budget, Core uses one anchored session summary that later turns merge into rather than re-derive. The summary is untrusted continuity prose: it records objective, constraints, work state, and next move, never carries authoritative `LABEL=value` values, artifact names, or completion state, and never selects instructions. It is produced only at or above a 16,384-token context, selects the largest pending-message prefix that fits that allocation, and runs outside the completed-run transaction. A missing structured call receives one fresh bounded retry; every other failure and a repeated failure fall back to deterministic history excerpts so a run outcome never depends on the summary.
- If a verified complete result exceeds the bounded chat response, the agent chooses a file type suited to the requested shape, creates and reopens one workspace deliverable, summarizes it concisely in chat, and declares the observed file. The workbook skill uses XLSX for oversized tabular results and must preserve every requested row instead of returning placeholders or an undeclared path.
- All static model instructions are Markdown assets under root `prompts/`. System, state, and recovery fragments are separate from Agent Skills-compatible `prompts/skills/<name>/SKILL.md` workflows. Core validates the skill name and description contract, exposes the metadata catalog, and progressively includes relevant skill bodies using the user task, attachment names, and explicit typed workflow state. Arbitrary file contents, stdout, stderr, and model text never select instructions. Typed runtime facts and schemas remain in TypeScript. Packaged prompt assets are offline resources covered by the package manifest; Windows verifies and read-locks every prompt before Core starts.

## Model And Asset Distribution

The first V1 package is self-contained and performs zero downloads on first launch. It includes only approved prompt assets, runtime assets, one generation model, the guest image, and required native helpers whose hashes appear in the package manifest. One Windows x64 package carries the pinned CUDA 13.1 binding and cuBLAS redistributables together with Vulkan; TypeScript attempts CUDA first and falls back to Vulkan so NVIDIA and supported AMD systems never require separate Vault Desk installations. The self-contained model exceeds NSIS's package limit, so the M3 Windows artifact is a signed copy-installed application directory with the same copy, restart, replacement, and removal lifecycle as the macOS application bundle; public release installers remain a separate credentialed distribution step.
...
Before the packaged host launches sensitive children, it verifies the sidecar and the signed-application-anchored resource manifest, verifies fixed native helpers and every prompt asset against that manifest, and retains non-writable read locks until shutdown so a user-writable copy cannot swap children or instructions after verification. The setup helper receives the same verification and lock before elevation. The webview retains the same fixed capability set and cannot invoke arbitrary process or setup operations. The desktop selects the inference envelope automatically: 10 GiB on Macs through 16 GB, 12 GiB through 24 GB, 16 GiB above 24 GB, and one selected Windows device's complete dedicated VRAM. Aggregate multi-device or unified and shared Windows memory readings are unsupported. An 8 GB Mac does not start inference and exposes a clear unsupported status. The worker fits the largest generation context inside the selected budget, capped at 64K on Macs through 32 GB unified memory and Windows GPUs through 24 GB dedicated VRAM, or 128K on Macs and Windows GPUs above those respective thresholds. It reports the actual allocation. It keeps the approved model resident after first use. Manual unload or Core shutdown terminates the complete worker process so all native model and context resources are reclaimed together; the next request launches and verifies it again.
