...
5. Atomically writes complete Python or Node source to a safe workspace-relative path, or runs a command shorter than 4,096 characters through `/bin/sh` from `/workspace`. A command that reaches the schema boundary is treated as potentially truncated and rejected before execution; a shell parse failure caused by an unterminated quoted string forces the next action into Python or Node source mode instead of asking for another shell command.
...
- Conversation context is rebuilt from durable messages and execution events. Core requests at most 32,768 output tokens and reserves up to that amount while keeping at least 4,096 tokens for the current prompt; the certified 8K minimum reserves 4,096 output tokens. It protects the current run and newest failed repair chain, anchors up to two recent user turns, and compacts older history without deleting originals. A generation-limit stop gets one newly traced recovery request for a small complete Python or Node action that incrementally creates or patches a persistent workspace file; repeated exhaustion fails explicitly without executing partial output.
