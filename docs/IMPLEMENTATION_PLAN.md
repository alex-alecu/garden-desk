...
- If an answer reaches its output limit, Core discards the partial text, compacts retained evidence, and requests one complete response-only turn. Successful runs independently expose the latest non-internal files observed in workspace deltas; no model-authored declaration or format-specific completion marker is required.
