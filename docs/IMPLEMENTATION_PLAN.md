...
5. Atomically writes complete Python or Node source to a safe workspace-relative path, or runs a command shorter than 4,096 characters through `/bin/sh` from `/workspace`. The structured grammar accepts one complete command without a grammar-level string cutoff, and Core validates the total before execution so a boundary-length result is treated as potentially truncated and rejected. Core also rejects a shell proposal that embeds Python or Node source before it reaches the VM. A shell parse failure caused by an unterminated quoted string forces the next action into Python or Node source mode instead of asking for another shell command. When a transaction, salary, advance, workbook, or spreadsheet task discovers an `.xlsx` path case-insensitively in a recorded command, source, or stdout observation, later turns enter the existing source-only XLSX workflow and count XLSX coverage independently of other formats.
