...
- Conversation context is rebuilt from durable messages and execution events. Core requests at most 32,768 output tokens for a new executable proposal and reserves up to that amount while keeping at least 4,096 tokens for the current prompt; failed or rejected repairs plus XLSX repair and continuation turns use 8,192 tokens, and forced final responses use 4,096. The certified 8K minimum therefore reserves 4,096 output tokens. Core protects the current run and newest failed repair chain, anchors up to two recent user turns, and compacts older history without deleting originals. Oversized current-run observations produce inspectable task, evidence, artifact, and warning ledgers in later traced prompts while exact execution bytes remain durable; those ledgers are anchored and merged forward deterministically rather than recomputed from every raw stream. A completed step is not rerun merely because its raw output left the live prompt. A generation-limit stop gets one newly traced 8,192-token recovery request for a small complete Python or Node action that incrementally creates or patches a persistent workspace file; repeated exhaustion fails explicitly without executing partial output.
- When a session outgrows its verbatim history budget, Core replaces the former omitted-message count with one anchored session summary that later turns merge into rather than re-derive. The summary is untrusted continuity prose: it records objective, constraints, work state, and next move, never carries authoritative `LABEL=value` values, artifact names, or completion state, and never selects instructions. It is produced only at or above a 16,384-token context and outside the run transaction, and any failure falls back to the deterministic history excerpts so a run outcome never depends on it.
