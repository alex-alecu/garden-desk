...
- Acceptance evidence: a packaged app on physical macOS and Windows; folder and attachment flows; direct and delegated image inspection; grouped session and workspace restoration; real multi-step Python, Node.js, and shell tasks; structural network denial; host-write and package-install denial; cancellation/restart recovery; and signed sidecar and guest-image verification. The two named domain skills also require final-prompt physical cases that load the domain skill before each format skill, cite both sides of each tested conflict, retain all required source values, state the applicable human-review limit, resist instructions embedded in source documents, and pass the unchanged canonical M3 platform gate.
...
- Explicitly not doing before V1: canonical document ingestion, OCR/layout routing, hybrid retrieval, deterministic citation verification, domain-specific Core behavior, domain workflows other than the two named prompt-only review skills, Knowledge Bundle import, external integrations, or model downloads.
