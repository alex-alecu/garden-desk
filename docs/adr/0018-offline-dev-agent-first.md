...
Core builds each Gemma request from durable conversation messages and execution events. It requests at most 32,768 output tokens and, while assembling context, reserves up to that amount while leaving at least 4,096 tokens for the current prompt; the certified 8K minimum therefore reserves 4,096 output tokens. It uses the actual allocated context when known, preserves the current run and newest unsuperseded failed repair in full, anchors the last two user turns, and summarizes older history without deleting stored originals. If mandatory repair context cannot fit, Core returns `agent_context_exhausted` rather than dropping required source or commands. If Gemma reaches the generation limit before producing a complete structured action, Core records the failed turn and makes one new traced request restricted to a small complete Python or Node source action that creates or patches one bounded part of a persistent workspace file. A repeated generation-limit stop returns an explicit failure instead of executing partial output.
