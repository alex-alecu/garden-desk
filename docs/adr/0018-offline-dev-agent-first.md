...
Core builds each Gemma request from durable conversation messages and execution events. It requests at most 32,768 output tokens for a new executable proposal, 8,192 for a failed or rejected repair or XLSX repair and continuation turn, and 4,096 for a forced final response. While assembling context, Core reserves up to the applicable amount while leaving at least 4,096 tokens for the current prompt; the certified 8K minimum therefore reserves 4,096 output tokens. It uses the actual allocated context when known, preserves the newest unsuperseded failed repair source exactly, anchors the last two user turns, and summarizes older history without deleting stored originals. Successful source, commands, stdout, and stderr share an allocation-derived live budget; overflow adds inspectable task, evidence, artifact, and warning ledgers before the next turn while retaining exact durable execution bytes. Each possible execution keeps a bounded ranked evidence share, so later turnover cannot displace earlier explicit identifiers or labels. A completed execution is not repeated merely because its raw context was compacted. When verbatim older chat no longer fits, Core uses one allocation-bounded anchored summary; it merges the largest fitting prefix of new messages and gives a missing structured call one fresh bounded retry before deterministic fallback. If mandatory repair context cannot fit, Core returns `agent_context_exhausted` rather than dropping required source or commands. If Gemma reaches the generation limit before producing a complete structured action, Core records the failed turn and makes one new traced 8,192-token request restricted to a complete Python or Node source action of at most 64 lines that creates or patches one bounded part of a persistent workspace file. A repeated generation-limit stop returns an explicit failure instead of executing partial output.

An exit-zero execution whose nonempty stdout reaches the typed `resource_limit` remains bounded evidence and may finalize without another source action. Empty or nonzero resource failures, timeouts, crashes, and stderr remain repair conditions.

When a verified complete result cannot fit within the bounded response field, the agent chooses the local file type that best preserves the requested result shape, creates and reopens one workspace deliverable, returns a concise chat summary, and declares the observed artifact. For oversized tabular workbook results, the active XLSX skill uses an XLSX deliverable rather than abbreviating rows or claiming an undeclared workspace file.
