...
Core sends the complete live message history and the current agent's generic tool definitions to Gemma for each turn. A run stops after at most 40 model turns and 24 started guest executions. Core validates each tool call before it can use the guest budget. Safe optional integers clamp to their configured bounds; wrong types, non-finite numbers, and unsafe integers fail. Python and Node source can be committed under `/workspace/steps/`. A later path-only call resolves and runs the exact committed UTF-8 bytes; Core records those resolved bytes with the execution. Shell source is executed as supplied. Syntax and runtime failures return their real interpreter output to the next turn. `read`, `glob`, `grep`, and `list` run inside the same guest roots. `read` streams bytes, decodes strict UTF-8 plain text, and fails for invalid UTF-8 or NUL bytes. Only the third consecutive byte-identical call is blocked with a change-approach reminder; no format, command-shape, or source-pattern policy rejects code before execution.
...
| 2026-08-22 | Added strict streamed text inspection, bounded JSON-aware output spill, measured session-summary queue behavior, and prompt-only format verification methods. |
| 2026-08-26 | Added committed-script path execution, failed-artifact recovery, and task-text-independent generated-file completion. |
