...
Gemma 4 reasoning is enabled through its supported chat wrapper. Only explicitly typed `thought` segments may cross the worker stream into bounded, transient desktop memory for display until the application closes. Those segments never enter the workspace database, conversation, agent event, or audit log. The matching inference event can store only the segment duration. Token counts and timing measurements cross in the terminal typed response and are aggregated into persisted numeric run metrics.
