...
5. Python and Node source can be committed under `/workspace/steps/`. A path-only call resolves the exact committed bytes before it uses the unchanged typed guest execution protocol. Execution and inspection tools run inside the guest; skill bodies load on demand through the generic `skill` tool; a depth-one child agent gets an isolated history while sharing the same session VM.
...
| 2026-08-22 | Recorded strict streamed text inspection, bounded output spill, and measured summary-queue behavior. |
