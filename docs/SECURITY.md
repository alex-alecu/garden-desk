...
Vault Core records every message, generated source or command, workspace path, summary, bounded logs, structured result, event, accepted deliverable, resource use, and termination reason. Workspace intermediates remain recoverable through the session workspace and debug snapshot but do not become artifact rows. Results and workspace deltas remain untrusted until schema, path, hash, size, protocol, and policy validation completes. See [ADR 0018](adr/0018-offline-dev-agent-first.md).

Only a successful final response can commit deliverables. For a task-named output, Core requires the matching safe current-run workspace path. It gives one generic recovery turn when it is absent, then returns a stable required-artifact failure. Core excludes internal tool, output-spill, and checkpoint paths. It accepts no suffix or format allowlist for safe task-named paths. Core selects the latest bytes, derives known MIME types from a trusted extension map, verifies the immutable content hash, and commits valid current-run artifacts with the response. Missing, oversized, removed, or failed-run files produce no card.
