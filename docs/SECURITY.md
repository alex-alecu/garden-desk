...
Vault Core records every message, generated source or command, workspace path, summary, bounded logs, structured result, event, accepted deliverable, resource use, and termination reason. A path-only Python or Node execution resolves the exact committed workspace bytes before guest execution, and Core records those bytes with the execution. Reusable scripts, notes, and checkpoints use `/workspace/steps/`. Workspace intermediates remain recoverable through the session workspace and debug snapshot but do not become artifact rows. Results and workspace deltas remain untrusted until schema, path, hash, size, protocol, and policy validation completes. See [ADR 0018](adr/0018-offline-dev-agent-first.md).
...
| 2026-08-22 | Added strict streamed text inspection and bounded output spill. |
| 2026-08-26 | Added committed-script execution records and failed-artifact recovery rules. |
