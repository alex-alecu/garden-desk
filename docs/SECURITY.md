...
The guest receives no user home, credentials, arbitrary host path, package manager access, host shell, generic Vault Core API, external connection broker, approval capability, or model endpoint. Its shell is guest-local and cannot cross the VM boundary. Vault Core mediates completions through the separately confined native inference worker and sends only validated execution requests to the guest. Code and dependencies cannot be installed. The generic `read` tool streams bytes and accepts strict UTF-8 plain text only. It rejects invalid UTF-8 and NUL bytes. Safe optional integers clamp to configured bounds; wrong types, non-finite numbers, and unsafe integers fail. Tool-result previews are limited to 50 KiB after JSON encoding. A complete oversized result enters the internal spill path only when all required chunks fit in the remaining guest-execution budget; no partial spill starts.

Vault Core records every message, generated source or command, execution path, summary, bounded logs, structured result, event, accepted deliverable, resource use, and termination reason. Python and Node calls accept relative workspace paths, normalized `/workspace/...` paths, and path-only `/source/...` paths. Workspace path-only execution resolves and records the exact committed bytes before guest execution. Direct source execution runs the live read-only file and records its absolute path with null source text; it does not claim exact-byte reproducibility. Reusable scripts, notes, and checkpoints use `/workspace/steps/`. Unsupported or corrupt model input is not trusted event detail: its durable event keeps only the tool name and call ID, and its failed tool result returns to the model without task termination. Workspace intermediates remain recoverable through the session workspace and debug snapshot but do not become artifact rows. Results and workspace deltas remain untrusted until schema, path, hash, size, protocol, and policy validation completes. See [ADR 0018](adr/0018-offline-dev-agent-first.md).

Only a successful final response can commit deliverables. Core excludes internal tool, output-spill, and checkpoint paths, selects the latest safe bytes, derives known MIME types from a trusted extension map, verifies the immutable content hash, and commits valid current-run artifacts with the response. A failed execution invalidates stale candidates and may retain changed safe bytes only for a later successful recovery execution. Missing, oversized, removed, or failed-run files produce no card. Task text, suffixes, format names, and model completion markers do not create a deliverable requirement.
