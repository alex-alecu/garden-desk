...
- Agents run in the background. After launching a group of agents, call `TaskOutput` once per agent with `block: true` and the timeout given for that step. Do not poll, sleep, schedule wakeups, use `Monitor`, or call `TaskOutput` a second time on the same agent.
...
1. Launch a haiku agent that returns whether the pull request is closed, is a draft, or already has a review comment from claude (check `gh pr view <PR> --comments`). Wait with timeout `60000`. Stop if any is true. Fallback: continue with the review.
