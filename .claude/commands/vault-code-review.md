...
- Stay inside the budget: read the diff and the files it touches, follow at most the direct references you need for a citation, and return before the timeout with what you have. Subagents never launch agents of their own.
...
1. Launch a haiku agent that returns whether the pull request is closed, is a draft, or already has a review comment from claude (check `gh pr view <PR> --comments`). Wait with timeout `45000`. Stop if any is true. Fallback: continue with the review.
