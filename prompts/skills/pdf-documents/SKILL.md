...
7. When the task explicitly expects invalid-input detection and requests a marker, catch the parse exception, print that exact marker to stdout, and exit normally with code 0. The marker is the completed validation result: do not repair the PDF, write an artifact, print a traceback, or execute again.
