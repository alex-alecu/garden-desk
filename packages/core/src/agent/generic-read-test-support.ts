...
export async function runGuestProgram(
  ...
  try {
    ...
    await writeFile(path, await script((await realpath(root)).replaceAll("\\", "/")));
    try {
...
export async function executeRead(
  ...
  return await runGuestProgram({ input: bytes }, async (guestRoot) =>
    (await readSource({ path: "input", ...params }))
