...
Buildroot 2026.05 produces the minimal Linux kernel and initramfs used by the no-network worker protocol probe. M0 selected the tool and image inputs; M1 owns the production build entrypoint and the macOS and Windows launchers. Buildroot is build tooling, not a runtime or shipped package manager. Its source archive is pinned by URL and SHA-256 in `manifest.json`.
...
Run `pnpm guest:build` for the probe image. The command downloads the pinned Buildroot source archive to `.generated/downloads` only when it is missing. To use another local archive, set `GARDEN_DESK_BUILDROOT_ARCHIVE` to its absolute path. The archive must match the manifest hash; use `-- --arch x86_64` for the Windows build.

Run `pnpm guest:build:agent` for the macOS M3 agent image. The command performs two independent builds, disables Docker networking for the second build, requires byte-identical outputs, and installs generated artifacts only under the ignored `.generated/agent/` tree.

Run `pnpm guest:build:agent:windows` for the Windows x86_64 M3 agent image. It applies the same independent online and offline build check and keeps the generated image ignored.
...
The manifest records reproducible arm64 and x86_64 kernel and initramfs hashes. Both M1 architecture builds were repeated after the production frame and denial probes were added; each second build had Docker networking disabled and matched byte-for-byte. The certified x86_64 hashes are kernel `ec0364eab93e9a12e4f5ef3008207331b03ef32a23dd9b0fc0f8c197fb126e45` and initramfs `cdf5a631ee8cc7aabb5def990de9beb922221acf5a46dc51ce2492498b225986`. Recording a digest does not by itself certify a platform: the current host must boot that exact guest and pass its no-network socket gate.
