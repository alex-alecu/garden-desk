################################################################################
# garden-desk-python-libraries
################################################################################

GARDEN_DESK_PYTHON_LIBRARIES_VERSION = 1
GARDEN_DESK_PYTHON_LIBRARIES_SOURCE = pypdf-6.14.2-py3-none-any.whl
GARDEN_DESK_PYTHON_LIBRARIES_SITE = https://files.pythonhosted.org/packages/49/e6/136aa8993a2ae7214e0b0ef2edaa0d2e08d1d4e4982635b08a835ff31ec8
GARDEN_DESK_PYTHON_LIBRARIES_EXTRA_DOWNLOADS = \
...
GARDEN_DESK_PYTHON_LIBRARIES_LICENSE = BSD-3-Clause and MIT and GPL-2.0-or-later-with-font-exception and Bitstream-Vera
GARDEN_DESK_PYTHON_LIBRARIES_LICENSE_FILES = site/pypdf-6.14.2.dist-info/licenses/LICENSE \
...
define GARDEN_DESK_PYTHON_LIBRARIES_EXTRACT_CMDS
	mkdir -p $(@D)/site
	$(foreach wheel,$(GARDEN_DESK_PYTHON_LIBRARIES_SOURCE) $(notdir $(GARDEN_DESK_PYTHON_LIBRARIES_EXTRA_DOWNLOADS)), \
		$(HOST_DIR)/bin/python3 -m zipfile -e $(GARDEN_DESK_PYTHON_LIBRARIES_DL_DIR)/$(wheel) $(@D)/site$(sep))
endef

define GARDEN_DESK_PYTHON_LIBRARIES_INSTALL_TARGET_CMDS
